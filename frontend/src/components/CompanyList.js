...
class CompanyList extends Component {
  ...
  getCompanyData = async () => {
    try {
      ...
      const companyResponse = await fetch('http://127.0.0.1:8000/api/company/');
  ...
  getBrandData = async () => {
    try {
      ...
      const brandResponse = await fetch('http://127.0.0.1:8000/api/brand/');
  ...
  // Render method: Define the structure of the rendered component
  render() {
    const { companies, brands } = this.state;
      ...
      <div className=' listbroder '>
          ...
          {companies.map((company) => (
            <div key={company.id} className='border-bottom rounded-sm p-3 pt-6  '>
              {company.image && (
                <img src={company.image} alt={company.name} style={{ width: '250px', height: '100%' }} />
              )}
